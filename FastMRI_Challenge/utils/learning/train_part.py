...
def train_epoch(args, epoch, model, data_loader, optimizer, loss_type):
    ...
    for iter, data in enumerate(data_loader):
        ...
        #crop output to make its shape equal to target - KDW
        tar_height = target.shape[1]
        tar_width = target.shape[2]
        out_height = output.shape[1]
        out_width = output.shape[2]
        
        output = output[..., (out_height - tar_height) // 2 : tar_height + (out_height - tar_height) // 2, (out_width - tar_width) // 2 : tar_width + (out_width - tar_width) // 2]
...
def train(args):
    ...
    train_loader = create_data_loaders(data_path = args.data_path_train, args = args, shuffle=False)
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        save_model(args, args.exp_dir, epoch + 1, model, optimizer, best_val_loss, False)
        print(
            f'model saved before validate! '
